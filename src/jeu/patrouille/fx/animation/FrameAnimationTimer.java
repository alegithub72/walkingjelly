...
/**
        if(target!=null ){
            Bounds shotSceneBound=shot.localToScene(shot.getImg().getBoundsInLocal());
            Bounds targetSceneBound=target.localToScene(target.getImg().getBoundsInLocal());
            if( !detectionRevelead 
                    && targetSceneBound.contains((shotSceneBound.getMinX()+(shotSceneBound.getWidth()/2)),(shotSceneBound.getMinY()+(shotSceneBound.getHeight()/2)))
                   // && targetSceneBound.intersects(shotSceneBound)
                    ){
               // System.out.println("INTERSECTION DETECTED!!!!!!!");
            CollisionSpriteEvent c=new CollisionSpriteEvent(source,
                    target,shot,null, CollisionSpriteEvent.COLLISION_SPRITE);
            detectionRevelead=true;
            source.fireEvent(c);                
            }
        ...
        if(i<=f2  && intervalTemp>this.interval) {
            ...
            if(this.ciclyc){
                if(i>f2) i=f1;
            }else if(!this.ciclyc && (i>f2) ) {
                //TODO ci deve essere un framme vuoto che corrisponde a f2
                //source.setFrame(i);
                source.removeThis();
                stop();
                i=f2;
